...
To use YRBA with SMB uploads configure the protocol in your `config.toml` to
SMB, and set up the username to log in with.
...
To authenticate as a guest user against the smb server set the username to
`guest`, and don't set a password field, or set `smb_password = ''`.
