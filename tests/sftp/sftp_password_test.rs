...
const CONFIG: &str = include_str!("./config.sftp.password.toml");
const TEMP_PATH: &str = "/tmp/yrba-test-sftp-password/";
...
fn test_sftp_password_backup() {
    ...
    // Setup config file
    let config_file_path = TEMP_PATH.to_owned() + "config.sftp.password.toml";
    ...
    sess.userauth_password("testuser", "Sup3rS3cur3").unwrap();
