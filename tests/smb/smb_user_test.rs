...
use smb::{
    Client, ClientConfig, CreateDisposition, Directory, FileAccessMask, FileCreateArgs,
    FileFullDirectoryInformation, ReadAt, Resource, Tree, UncPath,
};
use std::fs;
use std::fs::File;
...
const CONFIG: &str = include_str!("./config.smb.user.toml");
const TEMP_PATH: &str = "/tmp/yrba-test-smb-user/";
...
async fn test_smb_user_backup() {
    ...
    // Setup config file
    let config_file_path = TEMP_PATH.to_owned() + "config.smb.user.toml";
    ...
    let remote_address = "127.0.0.1:4455";
    let share_name = "content";
    let username = "testuser";
    let password: &str = "Sup3rS3cur3";
    let backup_directory_path = r"yrba-test-smb-user\out";
    ...
    let dir_create_path = &format!(
        "\\\\{}\\{}",
        "127.0.0.1:4455\\content\\yrba-test-smb-user\\out", uploaded_file.file_name
    );
