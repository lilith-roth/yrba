...
use crate::upload::utils::file_name::{generate_backup_name, get_all_backups_older_than_n_newest_backups, get_backup_name_stem};
...
pub fn file_copy_backup(backup_file_path: &Path, config: &Config) {
    ...
    let target_directory = remote_url.path();
    log::debug!(
        "File copy backup: {} -> {}",
        backup_file_path.display(),
        target_directory
    );
    let file_name = generate_backup_name(backup_file_path);
