...
pub(crate) fn create_tarball(path_to_backup: Box<&Path>) -> Result<PathBuf, std::io::Error> {
    ...
    let tar_gz = File::create(backup_archive_temp_file_path.clone()).expect("Could not generate filepath for temporary file!");
    ...
    let home_dir = binding.to_str().expect("Could not convert home directory path object to str!");
    ...
    match tar.finish() {
        ...
        Err(err) => {
            log::error!("Error finalizing tar archive {:?}\nError: {:?}", final_path_to_backup.as_os_str(), err);
            Err(err)
        },
    }
}
